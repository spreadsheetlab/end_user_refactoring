...
\todo{TODO: make this section one column only in length}
...
We begin by mapping OO smells to two education languages. Given that this is a first look into code smells in education languages, we limit our exploration to the 11 code smells found in other end-user programming languages. 
...
No demographic information was collected for any of the program authors in either language. 

\subsubsection{\ms}
We  gathered 17 programs from two data sources. The first source is a weekly robotics club for children aged 8 to 13, which yielded eight programs. 
...
To obtain a more diverse set of EV3 programs, we also solicited members of the EV3 programming group on Facebook to share their programs with us and refer to these as programs from the \emph{community}. Nine programs come from this second source. 


\subsubsection{Kodu}
We gathered 27  programs from two data sources. 
For the first data source, we ran a workshop at the Microsoft FUSE lab that introduced children to Kodu Game Lab in a series of three 3-hour sessions.  
...
Related to the current research are efforts on code smells within traditional languages; we start with the work of Fowler\cite{Fowl1999}. His book gives an overview of code smells and corresponding refactorings. 
