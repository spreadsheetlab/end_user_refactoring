...
\title{Spreadsheets are Code \large{\\Case studies on \ms~and Kodu}}
...
In this paper we broaden research on end-use code smells by examining the domain of educational programming languages. We gathered .... programs in \ms~and Microsoft's Kodu and studied the occurrence of code smells in these.

The results of this evaluation show that OO inspired smells in fact occur in both end-user languages: 88\% of the EV3 and 93\% of the Kodu programs contained at least one smell. This underlines the power of the code smells concept: they are applicable to visual languages aimed at education, which are quite different from the textual languages aimed at professional developers that the concept was initially designed for. 
...
\todo{fix later of potentially drop ``The rest of this paper''}
...
\section{Research Question and Studied Languages}
\label{sec:researchQuestion}
In Section~\ref{sec:background}, we summarized OO-inspired end-user smells from previous work. In both the Excel and the Yahoo! Pipes work, it was found that OO inspired smells were applied to end-user languages. This leads to the question how broadly applicable code smells are to end-user languages. Therefore, the aim of this paper is to explore that, by focusing on the research question: \textbf{Do code smells occur in programming languages aimed at education?}

We divide this question into three subquestions:

\begin{enumerate}
\item{How can we define code smells in \ms and Kodu?}
\item{Do the defined code smells occur in \ms and Kodu?}
\item{Are there domain specific, non OO inspired in \ms and Kodu?}
\end{enumerate}
To answer this research question, we study to new domains: a data-flow educational language for programming robots, \ms~software (EV3 for short),  and an event-driven educational language for building and playing video games, Kodu. 
