...
\subsubsection{Findings}
On average, the programs from children had 2.3 smells each and the programs from the community had 3.5 smells each, though we note that the programs from the community tend to be larger (e.g., an average of 101 rules vs. 42 rules). All smells appeared in at least one of the sampled programs except for deprecated code. In a conversation with the lead Kodu developer, we learned that none of the language features that have been deployed to the public were later deprecated. 
Next, we discuss the three most common smells found in the Kodu programs, appearing in 40\% or more of the sample. The remaining smells appear in 25\% or fewer programs. 
...
\todo{Can Table~\ref{} be updated to include columns for YP and Excel? I could get 
...
Special thanks to Stephen Coy for his insights into the Kodu language. This work is supported in part by  NSF SHF-EAGER-1446932 and the Harpole-Pentair endowment at Iowa State University.
