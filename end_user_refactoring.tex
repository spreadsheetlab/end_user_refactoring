...
Program                                                & 1   & 2  & 3   & 4   & 5   & 6   \\
\begin{tabular}[c]{@{}l@{}}Smell\\   Name\end{tabular} &     &    &     &     &     &     \\
...
Message Chain                                          &   &  &   &   &   &   \\
Many Parameters                                        &   &  &   &   &   &   \\
Duplicate Code                                         &   &  &   & \ding{51} & \ding{51} & \ding{51} \\
Dead Code                                              & \ding{51} &  & \ding{51} & \ding{51} &   &   \\
Unused Field                                           &   &  &   & \ding{51} &   &   \\
No-op                                                  &   &  & \ding{51} &   &   &  
\end{tabular}
\end{table}
...
Many of the \mbs~were relatively small, consisting of two or three blocks, or even 1 in some cases. You could say this is smelly, as understanding the \mb requires clicking it and that might not be worth it for small \mbs~. However, the \ms~interface does not allow regular blocks to be named, but it does allow this for \mbs. So by making a \mb, users can express the intent of a block. 


\begin{figure}
\caption{Compare these two blocks, the first one expresses exactly what the robot must do, but the second one expresses the intent (`weg' means flee). By using the \mb, even with one block, the program gets easier to read.}
\centering
...
We found a number of programs with \mbs~ no longer connected to the main program. This might be caused by the fact that EV3 interface does not help users in understanding what blocks are used. In the project properties screen, as shown in Figure \ref{fig:overview}
