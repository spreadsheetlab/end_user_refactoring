...
Stolee et al.~\cite{Stolee2011, StoleeTSE2013} treat a Yahoo!\ Pipes mashup as a program and each module as a method. Smells in this domain take the form of 

%Talk about adapting Fowler's definitions
\emph{Lazy Class} smell identifies classes, components or methods that "do not do enough"~\cite{Fowl99}. We defined three related smells specific for the mashup domain, identifying noisy modules, unnecessary modules, and unnecessary abstraction. The \emph{duplicate code} smell is rather vague, but applies to duplicate module parameters, modules, and structures. 

%Talk about smells unique to the domain
By exploring a large subset of the Yahoo!\ Pipes repository, Stolee et al. identified two smells based on the presence of broken data sources and the use of deprecated modules. 
